...
import {
  ...
  type ProjectEntry as ProjectEntryType,
  type McpServer,
...
class TestDataFacade extends BaseDataFacade {
  ...
  // Expose normalizeProjects for testing
  testNormalizeProjects(projects: unknown): ProjectEntryType[] {
...
describe("DataFacade", () => {
  ...
  describe("BaseDataFacade", () => {
    ...
    beforeEach(() => {
      ...
      mockConfig = {
        ...
        projects: [{ path: "C:\\Users\\test\\project1" }, { path: "C:\\Users\\test\\project2" }],
      };
  ...
  describe("normalizeProjects()", () => {
    ...
    it("should handle record format (actual .claude.json structure)", () => {
      const mockEnvInfo: EnvironmentInfo = {
        type: EnvironmentType.Linux,
        configPath: "/home/test/.claude.json",
      };
      ...
      const normalized = facade.testNormalizeProjects(rawProjects);
    ...
    it("should extract state from record format projects", () => {
      const mockEnvInfo: EnvironmentInfo = {
        type: EnvironmentType.Linux,
        configPath: "/home/test/.claude.json",
      };
    ...
    it("should extract mcpServers from record format projects", () => {
      const mockEnvInfo: EnvironmentInfo = {
        type: EnvironmentType.Linux,
        configPath: "/home/test/.claude.json",
      };
      ...
      const mcpServers = normalized[0].mcpServers as Record<string, McpServer>;
      assert.ok("test-server" in mcpServers);
    });

    it("should handle projects with minimal config", () => {
      const mockEnvInfo: EnvironmentInfo = {
        type: EnvironmentType.Linux,
        configPath: "/home/test/.claude.json",
      };
    ...
    it("should filter out invalid entries", () => {
      ...
      const mockConfig: ConfigReadResult = {
        config: {},
        projects: [{ path: "/valid" }, { path: "/another" }],
      };
      const facade = new TestDataFacade(mockEnvInfo, mockConfig);
      const projects = facade.getProjects();

      projects.then((result) => {
        assert.strictEqual(result.length, 2);
        assert.strictEqual(result[0]?.path, "/valid");
      });
    });
    ...
    it("should handle empty projects object", () => {
      ...
      const normalized = facade.testNormalizeProjects({});
    ...
    it("should handle null projects", () => {
      ...
      const normalized = facade.testNormalizeProjects(null);
      assert.strictEqual(normalized.length, 0);
    });
    ...
    it("should handle undefined projects", () => {
      ...
      const normalized = facade.testNormalizeProjects(undefined);
      assert.strictEqual(normalized.length, 0);
    });

    it("should skip non-object values in record format", () => {
      const mockEnvInfo: EnvironmentInfo = {
        type: EnvironmentType.Linux,
        configPath: "/home/test/.claude.json",
      };
  ...
  describe("cache behavior", () => {
    it("should expire cache after TTL", () => {
      ...
      // Read first time
      const projects1 = facade.getProjects();
      projects1.then((result) => {
      ...
      const projects2 = facade.getProjects();
      projects2.then((result) => {
        assert.strictEqual(result.length, 1); // Still cached
      });
    });
  });
});
