...
suite("FileFilter Tests", () => {
  ...
  suite("createFilterContext", () => {
    test("should create filter context with correct properties", () => {
      const context = createFilterContext("/home/test/.claude/settings.json", "settings.json", false, true, "/home/test/.claude", "/");
      ...
      assert.strictEqual(context.isInsideClaudeDir, true);
      assert.strictEqual(context.parentPath, "/home/test/.claude");
      assert.strictEqual(context.pathSep, "/");
    });
  });

  suite("isInsideClaudeDir", () => {
    test("should detect .claude directory in middle of path", () => {
      assert.strictEqual(isInsideClaudeDir("/home/test/.claude/settings.json", "/"), true);
    });

    test("should detect path ending with .claude", () => {
      assert.strictEqual(isInsideClaudeDir("/home/test/.claude", "/"), true);
    });

    test("should detect path ending with .claude without trailing separator", () => {
      assert.strictEqual(isInsideClaudeDir("/home/test/.claude", "/"), true);
    });

    test("should return false for non-.claude paths", () => {
      assert.strictEqual(isInsideClaudeDir("/home/test/project/src", "/"), false);
    });

    test("should work with Windows path separators", () => {
      assert.strictEqual(isInsideClaudeDir("C:\\Users\\test\\.claude\\settings.json", "\\"), true);
    });
  });
  ...
  suite("AllowAllFilter", () => {
    test("should include all files and directories", () => {
      ...
      const fileResult = filter.evaluate(createTestContext("test.txt", false, false));
      assert.strictEqual(fileResult.include, true);

      const dirResult = filter.evaluate(createTestContext("dirname", true, false));
  ...
  suite("AndFilter", () => {
    test("should include only when all filters include", () => {
      ...
      const result = andFilter.evaluate(createTestContext("test.txt", false, false));
      assert.strictEqual(result.include, true);
    });

    test("should exclude when any filter excludes", () => {
      const filter1 = new NamePatternFilter({ includePatterns: [/^test/] });
      const filter2 = new NamePatternFilter({ includePatterns: [/\.md$/] });
      const andFilter = new AndFilter([filter1, filter2]);
    ...
    test("should handle multiple filters", () => {
      ...
      const result = andFilter.evaluate(createTestContext("claude.json", false, false));
      assert.strictEqual(result.include, true);
    });
  });
  ...
  suite("OrFilter", () => {
    test("should include when any filter includes", () => {
      ...
      const result1 = orFilter.evaluate(createTestContext("test.txt", false, false));
      assert.strictEqual(result1.include, true);

      const result2 = orFilter.evaluate(createTestContext("hello.txt", false, false));
    ...
    test("should exclude when all filters exclude", () => {
      ...
      const result = orFilter.evaluate(createTestContext("world.txt", false, false));
      assert.strictEqual(result.include, false);
    });
  });
  ...
  suite("NotFilter", () => {
    test("should invert filter decision", () => {
      ...
      const result = notFilter.evaluate(createTestContext("test.txt", false, false));
      assert.strictEqual(result.include, false);
    });
  ...
  suite("NamePatternFilter", () => {
    test("should match include patterns", () => {
      ...
      const result1 = filter.evaluate(createTestContext("test.txt", false, false));
      assert.strictEqual(result1.include, true);

      const result2 = filter.evaluate(createTestContext("hello.txt", false, false));
      assert.strictEqual(result2.include, true);

      const result3 = filter.evaluate(createTestContext("test.json", false, false));
    ...
    test("should match exclude patterns", () => {
      ...
      const result2 = filter.evaluate(createTestContext("hello.tmp", false, false));
      assert.strictEqual(result2.include, false);

      const result3 = filter.evaluate(createTestContext("hello.txt", false, false));
      assert.strictEqual(result3.include, true);
    });
    ...
    test("should respect applyToDirectories flag", () => {
      ...
      const dirResult = filter.evaluate(createTestContext("src", true, false));
      assert.strictEqual(dirResult.include, true);

      const fileResult = filter.evaluate(createTestContext("src.txt", false, false));
  ...
  suite("ClaudeCodeFileFilter", () => {
    test("should include .claude directory", () => {
      ...
      const result = filter.evaluate(createTestContext(".claude", true, false));
      assert.strictEqual(result.include, true);
    });

    test("should include CLAUDE.md files", () => {
      const filter = new ClaudeCodeFileFilter();
      const result = filter.evaluate(createTestContext("CLAUDE.md", false, false));
      assert.strictEqual(result.include, true);
    });
    ...
    test("should include .mcp.json files", () => {
      const filter = new ClaudeCodeFileFilter();
      const result = filter.evaluate(createTestContext(".mcp.json", false, false));
      assert.strictEqual(result.include, true);
    });

    test("should include .claude.json files", () => {
      const filter = new ClaudeCodeFileFilter();
      const result = filter.evaluate(createTestContext(".claude.json", false, false));
      assert.strictEqual(result.include, true);
    });

    test("should exclude non-Claude files", () => {
      const filter = new ClaudeCodeFileFilter();
      const result = filter.evaluate(createTestContext("README.md", false, false));
      assert.strictEqual(result.include, false);
    });

    test("should exclude non-Claude directories", () => {
      const filter = new ClaudeCodeFileFilter();
      const result = filter.evaluate(createTestContext("src", true, false));
      assert.strictEqual(result.include, false);
    });

    test("should include all files inside .claude directory", () => {
      const filter = new ClaudeCodeFileFilter();
      const result = filter.evaluate(createTestContext("settings.json", false, true));
      assert.strictEqual(result.include, true);
    });

    test("should include all directories inside .claude directory", () => {
      const filter = new ClaudeCodeFileFilter();
      const result = filter.evaluate(createTestContext("skills", true, true));
      assert.strictEqual(result.include, true);
    });
  });
  ...
  suite("ProjectClaudeFileFilter", () => {
    test("should extend ClaudeCodeFileFilter behavior", () => {
      ...
      const claudeDirResult = filter.evaluate(createTestContext(".claude", true, false));
      assert.strictEqual(claudeDirResult.include, true);

      const claudeMdResult = filter.evaluate(createTestContext("CLAUDE.md", false, false));
  ...
  suite("FilterFactory.createClaudeFilterWithExtras", () => {
    ...
    test("should combine Claude filter with extra include patterns", () => {
      ...
      assert.ok(filter instanceof OrFilter);

      const context = createTestContext("include-test.txt", false, false);
    ...
    test("should apply extra exclude patterns", () => {
      ...
      const context1 = createTestContext("exclude-test.txt", false, false);
  ...
  suite("FilterFactory.fromConfig", () => {
    ...
    test("should add custom patterns to Claude filter", () => {
      ...
      const context = createTestContext("custom-file.txt", false, false);
      const result = filter.evaluate(context);
      assert.strictEqual(result.include, true);
    });
    ...
    test("should apply custom exclude patterns", () => {
      ...
      const context1 = createTestContext("test-file.txt", false, false);
      const result1 = filter.evaluate(context1);
      assert.strictEqual(result1.include, true);

      const context2 = createTestContext("test-temp", false, false);
  ...
  suite("Edge Cases", () => {
    ...
    test("should handle special characters in names", () => {
      const filter = new ClaudeCodeFileFilter();
      const context = createTestContext("CLAUDE.md.backup", false, false);
      const result = filter.evaluate(context);
      assert.strictEqual(result.include, false);
    });
    ...
    test("should handle nested .claude directories", () => {
      const filter = new ClaudeCodeFileFilter();
      const nestedPath = "/home/test/project/.claude/skills/subskill";
      const context = createFilterContext(nestedPath, "subskill", true, isInsideClaudeDir(nestedPath, "/"), "/home/test/project/.claude/skills", "/");
      const result = filter.evaluate(context);
      assert.strictEqual(result.include, true);
    });
    ...
    test("should handle case sensitivity correctly", () => {
      ...
      const lowerResult = filter.evaluate(createTestContext("claude.md", false, false));
      assert.strictEqual(lowerResult.include, false);

      const upperResult = filter.evaluate(createTestContext("CLAUDE.md", false, false));
