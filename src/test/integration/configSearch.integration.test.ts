...
import { EnvironmentType } from "../../services/environment.js";
...
describe("ConfigSearch Integration Tests", () => {
  ...
  it("should get environment info from native facade", async () => {
    ...
    assert.ok(validTypes.has(info.type), "Environment type should be valid");
  ...
  it("should have consistent facade info", async () => {
    ...
    for (const facade of facades) {
      ...
      // Environment type is always defined (non-optional EnvironmentType)
  ...
  it("should handle cross-platform path resolution", async () => {
    ...
    for (const facade of facades) {
      ...
      assert.ok(
        info.configPath.startsWith("/") ||
          info.configPath.includes(":") ||
          info.configPath.startsWith("\\\\"),
        "Config path should be absolute"
      );
      assert.ok(
        info.configPath.includes(".claude.json"),
        "Config path should point to .claude.json"
      );
    }
  });
});
